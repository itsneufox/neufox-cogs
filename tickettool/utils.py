    ...
    async def convert(
        ...
        try:
            ...
            raise commands.BadArgument(
                "Emoji Label must be an emoji followed by a label, and optionally by a description and a value (for rename ticket channel), separated by either `;`, `,`, `|`, or `-`."
            )
...
class CustomModalConverter(commands.Converter):
    async def convert(
        ...
        if len(argument_dict) > 5:
            raise commands.BadArgument("You can only have 5 text inputs.")
        for count, input in enumerate(argument_dict, start=1):
            count += 1
            for arg in required_arguments:
                if arg not in input:
                    raise commands.BadArgument(
                        f"The argument `/{count}/{arg}` is required in the YAML."
                    )
            for arg in input:
                if arg not in required_arguments + optional_arguments:
                    raise commands.BadArgument(
                        f"The argument `/{count}/{arg}` is invalid in the YAML. Check the spelling."
                    )
            if len(input["label"]) > 45:
                raise commands.BadArgument(
                    f"The argument `/modal/{count}/label` must be less than 45 characters long."
                )
            if "style" in input:
                ...
                try:
                    ...
                    raise commands.BadArgument(
                        f"The argument `/{count}/style` must be a number between 1 and 2."
                    )
                if not 1 <= style <= 2:
                    raise commands.BadArgument(
                        f"The argument `/{count}/style` must be a number between 1 and 2."
            ...
            if "required" in input:
                ...
                try:
                    ...
                    raise commands.BadArgument(
                        f"The argument `/{count}/required` must be a boolean (True or False)."
                    )
            else:
                input["required"] = True
