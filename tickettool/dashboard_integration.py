    ...
    async def rpc_callback_transcript(
        ...
        if not attachment_url.startswith("https://cdn.discordapp.com/attachments/"):
            if len(attachment_url.split("-")) >= 3:
                attachment_url = (
                    f"{attachment_url.split('-')[0]}/{attachment_url.split('-')[1]}"
                    f"/{'-'.join(attachment_url.split('-')[2:])}"
                )
            elif len(attachment_url.split("_")) >= 3:
                attachment_url = (
                    f"{attachment_url.split('_')[0]}/{attachment_url.split('_')[1]}"
                    f"/{'_'.join(attachment_url.split('_')[2:])}"
                )
        ...
        return {
            ...
            "web_content": {
                "source": (
                    '<iframe src="https://mahto.id/chat-exporter?url={{ attachment_url }}"'
