...
- Economy LWD$ rewards: optional rewards can be configured per chat or voice level.
...
- `[p]levelcoins add <chat|voice> <level> <amount>` - add or replace an Economy LWD$ reward.
- `[p]levelcoins remove <chat|voice> <level>` - remove an Economy LWD$ reward.
- `[p]levelcoins list` - show configured LWD$ rewards and Economy load status.
